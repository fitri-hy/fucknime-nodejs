...
class KomikFilterScreen extends StatefulWidget {
  const KomikFilterScreen({super.key});

  @override
  _KomikFilterScreenState createState() => _KomikFilterScreenState();
}

class _KomikFilterScreenState extends State<KomikFilterScreen> {
  int currentPage = 1;
  ...
  Future<void> fetchFilters() async {
    orders = await fetchFilterData('${ApiConfig.baseUrl}/komik/order');
    types = await fetchFilterData('${ApiConfig.baseUrl}/komik/type');
    statuses = await fetchFilterData('${ApiConfig.baseUrl}/komik/status');
    genres = await fetchFilterData('${ApiConfig.baseUrl}/komik/genre');
    setState(() {});
  }
  ...
  Future<List<dynamic>> fetchKomikList() async {
    final url =
        '${ApiConfig.baseUrl}/komik/list?order=${selectedOrder ?? ''}&genre=${selectedGenre ?? ''}&status=${selectedStatus ?? ''}&type=${selectedType ?? ''}&page=$currentPage';
              ...
              if (!snapshot.hasData || snapshot.data!.isEmpty) {
			    return SliverToBoxAdapter(
				  child: Padding(
				    ...
				    child: Center(
					  child: Text('Komik tidak ditemukan'),
				    ),
				  ),
			    );
			  }
